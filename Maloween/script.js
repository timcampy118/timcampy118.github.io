...
let usedNames = new Set();   // tracks prompt names used so far
let rerolledThisRound = false;
let rerollBtn; // define globally
let rerollCount = 0;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  easyModeToggle.checked = false; // start off
...
// Global palette (shared by all prompts)
const globalPalette = [
...
const drawings = Array(TOTAL_ROUNDS).fill(null); // dataURL per round
const rows = Array(TOTAL_ROUNDS).fill(null);      // comparison row canvas per round
const roundDims = Array(TOTAL_ROUNDS).fill(null); // { w, h } per round
...
/***************
 * ROUND UI (auto-size canvas to logo)
 ***************/
function setupRoundUI() {
  ...
  logoImg.onload = () => {
    ...
    brushWidthInput.value = String(brushSize);
...
function saveCurrentRound() {
  ...
  results[current] = {
    prompt: { ...selectedPrompts[current] }, // snapshot the prompt object
    drawing: drawingURL
  };
}
...
const fy = Math.floor(yCss * dpr);
  ...
  const target = getPixelAt(data, fx, fy, w);     // [r,g,b,a]
...
downloadAllBtn.addEventListener("click", () => {
  ...
  // 🕵️ DEBUG CHECK — show which prompts will be exported
  results.forEach((r, i) => {
    if (!r) {
    ...
    if (!src) {
      console.error(`❌ Round ${i}: src is undefined/null`, r.prompt);
    } else if (src === "null") {
      console.error(`❌ Round ${i}: src is literally 'null'`, r.prompt);
    }
  });

  // now actually perform export
  downloadFinalPoster();
});
...
function hasSignature(canvas) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // if any pixel isn't pure white, it's been drawn on
...
function isSignatureBlank(canvas) {
  ...
  for (let i = 3; i < pixelData.length; i += 4) {
    if (pixelData[i] !== 0) return false; // at least one pixel has content
  }
  return true;
}
...
/***************
 * SIGNATURE PAD — precise multi-click (no context scaling)
 ***************/
let sigDpr = window.devicePixelRatio || 1;
let signing = false, sx = 0, sy = 0;

function setupSignaturePad() {
  const cssW = signatureCanvas.width;   // 220
  const cssH = signatureCanvas.height;  // 60
  ...
  // Identity transform; draw in device pixels
...
function sigPointerToDevicePixels(e){
  const rect = signatureCanvas.getBoundingClientRect();
  const clientX = e.touches ? e.touches[0].clientX : e.clientX;
  const clientY = e.touches ? e.touches[0].clientY : e.clientY;
  return { x: (clientX - rect.left) * sigDpr, y: (clientY - rect.top) * sigDpr };
}
function sigStart(e){ e.preventDefault(); signing = true; const p = sigPointerToDevicePixels(e); sx=p.x; sy=p.y; }
function sigMove(e){
  if(!signing) return; e.preventDefault();
  const p = sigPointerToDevicePixels(e);
  sigCtx.beginPath(); sigCtx.moveTo(sx, sy); sigCtx.lineTo(p.x, p.y); sigCtx.stroke();
  sx = p.x; sy = p.y;
}
function sigEnd(){ signing = false; }
...
ctx.fillText(summaryText, MARGIN_X, OUT_H - FOOTER_H / 2);
    ...
    // === Signature at bottom ===
    const sigPad = 16;
    const sigDpr = window.devicePixelRatio || 1;
